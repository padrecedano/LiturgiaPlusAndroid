...
@UnstableApi
class TtsPlayerForApi26
@RequiresApi(Build.VERSION_CODES.O) // Marcar que la clase ahora tiene dependencias de API 26+
constructor(
    looper: Looper,
    private val context: Context,
    initialText: String,
    internal var overallProgressListener: (currentSegment: Int, totalSegments: Int) -> Unit = { _, _ -> },
    ...
    // Este listener recibirá el progreso (0.0 a 1.0), el tiempo de progreso y la duración total del segmento actual
    //private val segmentProgressListener: (progressPercent: Float, progressString: String, durationString: String) -> Unit
) : SimpleBasePlayer(looper, Clock.DEFAULT), OnInitListener {
    ...
    private var pendingTextToSet: String? = initialText
    ...
    private var fullTextToSpeak: String = initialText // Para almacenar el texto completo
...
// Dentro de addListener(object : Player.Listener { ... override fun onPlayWhenReadyChanged(...) { ...
            ...
            override fun onPlayWhenReadyChanged(playWhenReady: Boolean, reason: Int) {
                ...
                if (playWhenReady) {
                    Timber.d("TtsPlayerForApi26.onPlayWhenReadyChanged(true): SBP state is ${this@TtsPlayerForApi26.playbackState}")
                    // Si SBP está listo o bufferizando, y ahora debemos reproducir, iniciamos el habla.
                    if (this@TtsPlayerForApi26.playbackState == Player.STATE_READY || this@TtsPlayerForApi26.playbackState == Player.STATE_BUFFERING) {
                        speakSegmentAtIndex(currentMediaItemIndex)
                    } else if (this@TtsPlayerForApi26.playbackState == Player.STATE_ENDED) {
    ...
    private fun updateReportedPlayerState(
        ...
        newStateBuilder
            ...
            .setPlayWhenReady(
                this.intendedPlayWhenReady,
                if (this.intendedPlayWhenReady != oldPlayWhenReady) Player.PLAY_WHEN_READY_CHANGE_REASON_USER_REQUEST else this.playWhenReadyChangeReason
            )
    ...
    fun onInitt(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            Timber.d("TTS engine initialized (API 26+).")
            ttsInitialized = true
            val locSpanish = Locale("spa", "ESP")
            val langResult = textToSpeechEngine.setLanguage(locSpanish)

            if (langResult == TextToSpeech.LANG_MISSING_DATA || langResult == TextToSpeech.LANG_NOT_SUPPORTED) {
                Timber.e("TTS Language (Spanish) is not supported. Result: $langResult")
                updateReportedPlayerState(
                ...
                Timber.d("TTS language set successfully.")
            }


// En TtsPlayerForApi26.kt
// Dentro de override fun onInit(status: Int) { ...
            ...
            textToSpeechEngine.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                override fun onStart(utteranceId: String) {
                    ...
                    mainHandler.post {
                        Timber.i("TTS onStart: $utteranceId, currentSbpIndex: $currentMediaItemIndex")
                ...
                override fun onDone(utteranceId: String) {
                    mainHandler.post {
                        Timber.i("TTS onDone: $utteranceId, currentSbpIndex: $currentMediaItemIndex")
                        if (utteranceId == currentUtteranceId) {
                            segmentProgressListener.invoke(
                                1f,
                                formatMillis(currentSegmentEstimatedDurationMs),
                            ...
                            // La lógica de SBP se encargará de avanzar o terminar
                        }
                    }
                }

                // Método onError deprecado (requerido para la implementación)
                @Deprecated("deprecated in API level 21")
                override fun onError(utteranceId: String) {
                    mainHandler.post {
                        Timber.e("TTS onError (deprecated) for utteranceId: $utteranceId. Use onError(String, Int) instead.")
                        // Puedes llamar al otro onError con un código de error genérico si lo deseas,
                        // o manejarlo de forma similar.
                        this.onError(utteranceId, TextToSpeech.ERROR) // Redirigir o manejar
                    }
                }

                // Método onError más nuevo (API 21+)
                override fun onError(utteranceId: String?, errorCode: Int) {
                    mainHandler.post {
                        Timber.e("TTS onError (with code) for utteranceId: $utteranceId, errorCode: $errorCode")
                        segmentProgressListener.invoke(
                            0f,
                            formatMillis(0),
                            formatMillis(currentSegmentEstimatedDurationMs)
                        ...
                        updateReportedPlayerState(
                            playerErrorUpdate = PlaybackException(
                                "TTS playback error code $errorCode for $utteranceId",
                                null,
                                PlaybackException.ERROR_CODE_AUDIO_TRACK_INIT_FAILED
                            ),
                            playbackStateUpdate = Player.STATE_IDLE
                ...
                override fun onRangeStart(utteranceId: String?, start: Int, end: Int, frame: Int) {
                    if (utteranceId == currentUtteranceId) {
                        ...
                        if (currentIndex != C.INDEX_UNSET && currentIndex < localSegmentTexts.size) {
                            val segmentLength = localSegmentTexts[currentIndex].length
                            if (segmentLength > 0) {
...
// ...
            pendingTextToSet?.let { text ->
                Timber.d("Processing pending text after TTS init: \"$text\"")
            ...
            Timber.e("TTS engine initialization FAILED! Status: $status")
            ttsInitialized = false
            updateReportedPlayerState(
                playerErrorUpdate = PlaybackException(
                    "TTS Init Failed, status: $status",
                    null,
                    PlaybackException.ERROR_CODE_AUDIO_TRACK_INIT_FAILED
                ),
                playbackStateUpdate = Player.STATE_IDLE
            ...
            textToSpeechEngine.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                override fun onStart(utteranceId: String) {
                    ...
                    mainHandler.post { // Asegurar ejecución en el hilo del Looper de SimpleBasePlayer
                        // Timber.tag(TAG).i("TTS onStart: $utteranceId, currentSbpIndex: $currentMediaItemIndex, SBP State: ${this@TtsPlayerForApi26.playbackState}")
                        if (utteranceId == currentUtteranceId && this@TtsPlayerForApi26.playbackState == Player.STATE_BUFFERING) {
                            // Si estábamos en BUFFERING (porque llamamos a speakSegmentAtIndex), ahora estamos listos.
                            updateReportedPlayerState(playbackStateUpdate = Player.STATE_READY)
                        } else if (utteranceId == currentUtteranceId && this@TtsPlayerForApi26.playbackState == Player.STATE_IDLE && intendedPlayWhenReady) {
                ...
                override fun onError(utteranceId: String?, errorCode: Int) {
                    mainHandler.post {
                        ...
                        segmentProgressListener.invoke(
                            0f, // Sin progreso
                            formatMillis(0),
                            formatMillis(currentSegmentEstimatedDurationMs) // Duración estimada original
                        )
                        updateReportedPlayerState(
                            playerErrorUpdate = PlaybackException(
...
// ... (otras propiedades y métodos) ...
    ...
    private fun setContentInternal(text: String) {
        ...
        overallProgressListener.invoke(currentMediaItemIndex.coerceAtLeast(0), mediaItemCount)
    }
    ...
    fun setTextAndMediaItems(
        ...
        mainHandler.post { // Asegura que las actualizaciones de estado y SBP se hagan en el hilo principal.
            ...
            if (resetPosition) {
                ...
                Timber.d("Posición de reproducción interna reseteada. currentMediaItemIndex: $currentMediaItemIndex")
            }
