...
class SyncRepositoryImpl @Inject constructor(
    ...
    override suspend fun getSyncc(syncRequest: SyncRequest): SyncResponseNew {
        // var syncResponse: SyncResponse
        var syncResponseNew: SyncResponseNew

        if (!syncRequest.hasInitialSync) {
            //if (!syncRequest.hasInitialSync||true) {
            //syncResponse = syncFactory.create(Source.NETWORK).getSync(syncRequest)
            syncResponseNew = syncFactory.create(Source.LOCAL).getSyncc(syncRequest)
            if (syncResponseNew.allToday.isEmpty()) {
                //syncResponse = syncFactory.create(Source.FIREBASE).getSync(syncRequest)
                syncResponseNew = syncFactory.create(Source.FIREBASE).getSyncc(syncRequest)

            }
            if (syncResponseNew.allToday.isNotEmpty()) {
                syncFactory.create(Source.LOCAL).addSyncc(syncResponseNew)
            }
        ...
        return syncFactory.create(Source.LOCAL).getSyncc(syncRequest)
    }

    suspend fun getSynccX(syncRequest: SyncRequest): SyncResponseNew {
